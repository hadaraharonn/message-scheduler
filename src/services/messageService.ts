...
class MessageService {
    ...
    private readonly maxMessageDeletionRetries = config.maxMessageDeletionRetries as number;
    ...
    public async processMessages(): Promise<void> {
        try {
            ...
            for (const messageDTO of messages) {
                ...
                const lockAcquired = await this.redisApi.acquireLock(lockKey, lockValue);
                if (lockAcquired) {
                    try {
                        await this.messageHandler.handleMessage(messageDTO.message);
                        await this.deleteMessageWithRetries(messageDTO, this.maxMessageDeletionRetries);
                    } finally {
    ...
    private async deleteMessageWithRetries(messageDTO: MessageDTO, retries: number): Promise<void> {
        for (let attempt = 1; attempt <= retries; ++attempt) {
            try {
                await this.redisApi.deleteMessage(messageDTO);
                return;
            } catch (error) {
                console.error(`Attempt ${attempt} to delete messageId ${messageDTO.id} failed`, error);
                if (attempt === retries)
                    throw error;
            }
        }
