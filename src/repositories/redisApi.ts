...
class RedisApi {
    ...
    public async addMessageToSortedSet(messageDTO: MessageDTO): Promise<void> {
        try {
            ...
            await this.redisClient.zadd(this.scheduledMessages, messageDTO.timestamp.getTime().toString(), messageJson);
        } catch (error) {
    ...
    public async popMessage(): Promise<MessageDTO | null> {
        try {
            const now = Date.now();
            const messages = await this.redisClient.zrangebyscore(this.scheduledMessages, 0, now, 'LIMIT', 0, 1);
    ...
    public async acquireLock(message: string): Promise<boolean> {
        try {
            ...
            const lockAcquired = await this.redisClient.set(lockKey, 'locked', 'NX', 'EX', 60);
